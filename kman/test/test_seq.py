...
def test_SequenceCoords():
	try: SequenceCoords("chr1", -1, 1, SequenceCoords.STRAND.MINUS)
	except AssertionError as e: pass
	else: assert False, "start must be tested to be >=0"

	try: SequenceCoords("chr1", 0, -1, SequenceCoords.STRAND.MINUS)
	except AssertionError as e: pass
	else: assert False, "end must be tested to be >=0"

	try: SequenceCoords("chr1", 0, 1, "minus")
	except AssertionError as e: pass
	else: assert False, "strand must be tested to be from SequenceCoords.STRAND"
...
def test_KMer():
	try: KMer("chr1", -1, 10, "ACGATCGATCG")
	except AssertionError as e: pass
	else: assert False, "start must be tested to be >=0"

	try: KMer("chr1", 10, -1, "ACGATCGATCG")
	except AssertionError as e: pass
	else: assert False, "end must be tested to be >=0"

	try: KMer("chr1", 0, 11, "ACGATCGATCG", strand = "minus")
	except AssertionError as e: pass
	else: assert False, "strand must be tested to be from SequenceCoords.STRAND"

	try: KMer("chr1", 0, 11, "ACGATCGATCG", t = "DNA")
	except AssertionError as e: pass
	else: assert False, "nucl. acid type must be tested to be from om.NATYPES"

	try: KMer("chr1", 0, 1, "ACGATCGATCG")
	except AssertionError as e: pass
	else: assert False, "length must be tested for match"
...
def test_Sequence():
	...
	klist = [[KMer("ref", 0, 0+4, "ACGA"), KMer("ref", 1, 1+4, "CGAT")],
	...
	assert klist == [list(g) for g in s.kmerator_batched(
		s.text, 4, s.natype, 5, s.name)]
	...
	assert klist == [list(g) for g in s.kmerator_batched(
		s.text, 4, s.natype, 5, s.name, True)]

def test_SequenceCount():
	try: SequenceCount("ACGATCGATCG",  [1, 2, 3], "DNA")
	except AssertionError as e: pass
	else: assert False, "nucl. acid type must be tested to be from om.NATYPES"
